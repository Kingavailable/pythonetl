...
# 路径请自行改到配置文件中
files = fu.get_dir_files_list(conf.backend_logs_data_root_path)
...
target_db_util.check_table_exists_and_create(
    conf.target_db_name,
    conf.target_backend_logs_table_name,
...
processed_files = get_processed_files(
    ...
    table_name=conf.metadata_backend_logs_table_name,
    create_cols=conf.metadata_backend_logs_table_create_cols
)
...
backend_logs_write_f = open(
    conf.backend_logs_output_csv_root_path + conf.backend_logs_output_csv_file_name,
    "a",
    encoding="UTF-8"
)
